...
class RobotArmEnvV0(gym.Env):
    ...
    def render(self):
        # print("Render is not implemented!!")
        self.update_fig()
    ...
    def create_fig(self):
        P = self.forward_kinematics(self.theta)
        init_x = np.array([P[0][0][3],P[1][0][3],P[2][0][3]])
        init_y = np.array([P[0][1][3],P[1][1][3],P[2][1][3]])
        tar_x = np.array(self.target_pos[0])
        tar_y = np.array(self.target_pos[1])
        
        plt.ion()
        self.figure, self.ax = plt.subplots(figsize=(8,6))
        self.line1, = self.ax.plot(init_x, init_y, marker="o")
        self.cir1,  = self.ax.plot(tar_x, tar_y, marker="o", markersize=14)
        self.ax.set_xlim([-300,300])
        self.ax.set_ylim([-300,300])

        plt.title("PPO(robot arm)",fontsize=25)

    def update_fig(self):
        # リンク
        P = self.forward_kinematics(self.theta)
        updated_link_x = np.array([P[0][0][3],P[1][0][3],P[2][0][3]])
        updated_link_y = np.array([P[0][1][3],P[1][1][3],P[2][1][3]])
        self.line1.set_xdata(updated_link_x)
        self.line1.set_ydata(updated_link_y)
        # 目標位置
        updated_tar_x = np.array(self.target_pos[0])
        updated_tar_y = np.array(self.target_pos[1])
        self.cir1.set_xdata(updated_tar_x)
        self.cir1.set_ydata(updated_tar_y)
    ...
    def close(self):
        print("Close is not inplemented!!")
        #self.close_plot
        # if self.viewer != None:
        #     pygame.quit()
